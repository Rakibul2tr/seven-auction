...
const Auction = () => {
  ...
  const filterlive = tabeldatas.filter((item) => {
    return item.status === "live";
  });
  const filtersold = tabeldatas.filter((item) => {
    return item.status === "sold";
  });
  const filterunsold = tabeldatas.filter((item) => {
    ...
    <Container style={{ background: "#F6F8FA" }} className="min-vh-100">
        ...
        <Row className="pt-3">
          <div className="top_auction_section">
            ...
            <div className="auction_item">
              <p>totla</p>
              <h6 className="commonColor">{auctionList?.totalAuctions}</h6>
            </div>
            <div className="auction_item">
              <p>sold</p>
              <h6 className="lighgreen">{auctionList?.soldAuctions}</h6>
            </div>
            <div className="auction_item">
              <p>unsold</p>
              <h6 className="wornigcolor">{auctionList?.unsoldAuctions}</h6>
            </div>
            <div className="auction_item">
              <p>live</p>
              <h6 className="rejeactColor">{auctionList?.liveAuctions}</h6>
            </div>
          </div>
        </Row>
        <Row>
            ...
            <div className="inquires_nav d-flex justify-content-between">
              <div className="request_menu pt-5 ">
                ...
                <button
                  ...
                  className={
                    menuChange === "live" ? "inqiresactive && won_btn" : null
                  }
                >
                  live
                </button>
                <button
                  ...
                  className={
                    menuChange === "sold" ? "inqiresactive && lost_btn" : null
                  }
                >
                  sold
                </button>
                <button
                  ...
                  className={
                    menuChange === "unsold" ? "inqiresactive && lost_btn" : null
                  }
                >
                  unsold
                </button>
      ...
      <Row className="pb-5">
        <div className="table_section">
          ...
          {menuChange === "unsold" && (
            <AuctionUnsold filterunsold={filterunsold} />
          )}
        </div>
      </Row>
    </Container>
