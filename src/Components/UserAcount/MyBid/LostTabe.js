...
const LostTabe = (props) => {
    const [LostAuction,setLostAuction]=useState([])
    fetch(`https://seven-auction.herokuapp.com/api/user/my-bids?result=loss`)
    .then(res=>res.json())
    .then(data=>setLostAuction(data))
    return (
        <div className="bid_car_aria">
                <div className="row">
                {
                        LostAuction.length >0? props.bidedAuctions.map(car=>(<Col md={4} className="mb-5" key={car._id}>
